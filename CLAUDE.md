...
- v1.40.0 : le flash de navigation touchait aussi les écrans mono-usage non couverts par le correctif précédent (`/tables/overflow/[assignmentId]`, `/tables/move-guest/[guestId]`, `/tables/move/[invitationId]`, `/tables/move-multiple`, `/checkin/[invitationId]/merge`, `/approbations/[id]/assign`) : même correctif, TopBar toujours visible pendant le chargement. **Bug de données corrigé** (`0048_unplanned_guest_never_counts_toward_prevu.sql`, appliquée et vérifiée en production Supabase le 03/09/2026, `SECURITY INVOKER`) : `guests.is_unplanned` distingue désormais un accompagnant ajouté via `add_unplanned_arrival` (jamais compté dans `nombre_prevu`) d'un guest planifié — `set_guest_arrival_status`, `remove_invitation_member` et `split_guest_to_new_invitation` ignorent ce delta pour lui, quel que soit son `arrival_status` ; corrige le cas trouvé par Gersom où `nombre_prevu` retombait à 0 après avoir marqué de tels accompagnants "ne viendra pas". `agent_checkin` gagne `viewAgenda` en lecture seule (jamais `manageAgenda`) : sa barre du bas remplace Staff par Agenda (`/staff` reste atteignable par le badge QR "STAFF"). `/dashboard` ouvre aussi sa carte Staff au `placeur` (comme au directeur), sans changer ce que `/staff` lui montre ensuite (`viewAllStaff` inchangée). Le splash avant connexion (`app/page.tsx`) affiche désormais la version applicative en bas à droite, importée directement de `package.json` (jamais une variable dupliquée). `/checkin/[invitationId]` resserré (mêmes leviers que `/dashboard` v1.34.0) pour tenir sans défiler sur une fiche simple.
