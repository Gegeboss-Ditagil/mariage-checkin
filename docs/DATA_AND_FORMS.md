...
| Invité surprise (photo + SMS/WhatsApp d'approbation) | guest_approval_requests, Storage (bucket privé), audit | Admin, directeur, placeur |
...
| Assigner une table à un invité surprise approuvé | guest_approval_requests, invitations, audit | Admin, directeur, placeur |
...
Invité surprise avec approbation SMS/WhatsApp à distance (`/scan` → `/scan/guest-approval`, v1.27.0) : photo (`<input type="file" capture="environment">`, une seule prise), côté (Nelly/Gégé), nom, nombre d'invités ; validation serveur dans `POST /api/guest-approvals` (capacité `guestApproval` — admin/directeur/placeur, jamais agent scan ni visibilité) ; upload dans le bucket **privé** `guest-approval-photos` (jamais public, URL toujours signée côté serveur, 1h de validité — voir `lib/guestApprovalPhotos.ts`) ; numéro de l'approbateur lu dans `guest_approvers` (config, pas une variable d'environnement, pour que Gersom puisse le changer depuis Supabase sans redéploiement) ; SMS **et** WhatsApp envoyés en parallèle via l'API REST Twilio (`lib/twilio.ts`, `lib/guestApprovalNotify.ts`), chacun best-effort (l'échec de l'un ne bloque jamais l'autre) — **texte seul sur les deux canaux, jamais de MMS/média** (un numéro Twilio français ne supporte pas les MMS ; un message WhatsApp initié par l'app doit utiliser un Content Template Twilio pré-approuvé par Meta, `TWILIO_WHATSAPP_CONTENT_SID_REQUEST`, jamais de texte libre). Les deux canaux contiennent un lien vers `/approve/[token]` qui affiche la photo à l'ouverture. La demande créée n'écrit **aucune** table/invitation existante — c'est un nouvel enregistrement dans `guest_approval_requests` uniquement.
...
Dans les deux cas : l'update est gardé par `statut = 'en_attente'` dans la clause `WHERE` (atomique, une deuxième décision sur une demande déjà tranchée est refusée — `409 already_decided` côté web, message « déjà traité » côté WhatsApp) ; effet secondaire : `audit_logs` (`action = 'guest_approval_decided'`, avec le canal dans `details.via`), colonne `decided_via` (`'web'`/`'whatsapp'`) sur `guest_approval_requests`, et un SMS de confirmation à l'approbateur (nombre de places de réserve restantes si approuvé). `guest_approval_requests` n'a **aucune policy RLS anon** (contrairement aux tables opérationnelles historiques) : exposer `token` via une lecture anon casserait la confidentialité du lien — tous les accès passent par des routes API côté serveur (client `service_role`).

Assigner une table (`/approbations/[id]/assign`) : champ `table_id`, sélecteur `TablePicker` (mêmes capacités en direct que `/tables/move/[invitationId]`) ; validation serveur dans `POST /api/guest-approvals/[id]/assign-table` (capacité `guestApproval` — **volontairement pas** `addInvitation`, réservée à l'admin, voir `/api/invitations/add`) ; fonction SQL `assign_table_to_guest_approval` (`0032_guest_approvals.sql`) : refuse une demande pas encore `approuve` (409) ou déjà assignée (409), crée l'invitation, marque la demande assignée, journalise (`audit_logs`, `action = 'guest_approval_assigned'`) ; effet secondaire : SMS de rapport à `festin_directors` (nom, table, places de réserve restantes) — pré-remplie avec Rémy Landu et Tuzola (`0033_festin_directors_contacts.sql`).
