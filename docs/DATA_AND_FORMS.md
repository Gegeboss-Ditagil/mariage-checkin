...
| Invité surprise (photo + SMS d'approbation) | guest_approval_requests, Storage (bucket privé), audit | Admin, directeur, placeur |
| Décider (approuver/refuser) un invité surprise | guest_approval_requests, audit | Public (token secret, sans connexion) |
...
Invité surprise avec approbation SMS à distance (`/scan` → `/scan/guest-approval`, v1.27.0) : photo (`<input type="file" capture="environment">`, une seule prise), côté (Nelly/Gégé), nom, nombre d'invités ; validation serveur dans `POST /api/guest-approvals` (capacité `guestApproval` — admin/directeur/placeur, jamais agent scan ni visibilité) ; upload dans le bucket **privé** `guest-approval-photos` (jamais public, URL toujours signée côté serveur, 1h de validité — voir `lib/guestApprovalPhotos.ts`) ; numéro de l'approbateur lu dans `guest_approvers` (config, pas une variable d'environnement, pour que Gersom puisse le changer depuis Supabase sans redéploiement) ; SMS envoyé via l'API REST Twilio (`lib/twilio.ts`, `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_PHONE_NUMBER` — variables Vercel à ajouter, voir `CHANGELOG.md` v1.27.0) — **texte seul, jamais de MMS** (un numéro Twilio français n'en supporte pas), le SMS contient un lien vers `/approve/[token]` qui affiche la photo à l'ouverture. La demande créée n'écrit **aucune** table/invitation existante — c'est un nouvel enregistrement dans `guest_approval_requests` uniquement.

Décision (`/approve/[token]`, page **publique**, sans connexion) : `POST /api/public/guest-approvals/[token]/decide`, champ `decision` (`approuve`/`refuse`) ; la connaissance du token EST l'autorisation — aucune session, aucune vérification de capacité ; l'update est gardé par `statut = 'en_attente'` dans la clause `WHERE` (atomique, un deuxième clic sur un token déjà tranché reçoit `409 already_decided`) ; effet secondaire : `audit_logs` (`action = 'guest_approval_decided'`), et un SMS de confirmation à l'approbateur (nombre de places de réserve restantes si approuvé). `guest_approval_requests` n'a **aucune policy RLS anon** (contrairement aux tables opérationnelles historiques) : exposer `token` via une lecture anon casserait la confidentialité du lien SMS — tous les accès passent par des routes API côté serveur (client `service_role`).

Assigner une table (`/approbations/[id]/assign`) : champ `table_id`, sélecteur `TablePicker` (mêmes capacités en direct que `/tables/move/[invitationId]`) ; validation serveur dans `POST /api/guest-approvals/[id]/assign-table` (capacité `guestApproval` — **volontairement pas** `addInvitation`, réservée à l'admin, voir `/api/invitations/add`) ; fonction SQL `assign_table_to_guest_approval` (`0032_guest_approvals.sql`) : refuse une demande pas encore `approuve` (409) ou déjà assignée (409), crée l'invitation, marque la demande assignée, journalise (`audit_logs`, `action = 'guest_approval_assigned'`) ; effet secondaire : SMS de rapport à `festin_directors` (nom, table, places de réserve restantes) — table vide par défaut (numéros de Remy/Tuzola pas encore confirmés), no-op silencieux tant qu'elle l'est.
