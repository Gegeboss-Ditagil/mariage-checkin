...
- Après approbation, l'approbateur reçoit un SMS de confirmation indiquant combien de places de réserve il reste. Après assignation de table, un SMS de rapport part vers le directeur de festin (table `festin_directors` : nom de l'approbateur qui a validé, nombre de places, table assignée, places de réserve restantes) — pré-remplie avec Rémy Landu et Tuzola (`0033_festin_directors_contacts.sql`) ; reste un no-op silencieux (aucune erreur ni blocage) si cette table venait à être vidée.
