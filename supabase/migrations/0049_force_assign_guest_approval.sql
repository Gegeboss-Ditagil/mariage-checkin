-- ============================================================================
-- v1.41.0 — Forçage d'assignation sur une table pleine (retour de Gersom le
-- 03/09/2026) : quand l'événement est plein, l'écran d'assignation d'un invité
-- surprise approuvé ne proposait que les tables avec assez de places libres —
-- et le RPC refusait tout dépassement (`target_capacity_exceeded`). Résultat :
-- plus aucune table choisissable, l'invité approuvé restait sans place.
--
-- Ajoute `p_force` (défaut false, comportement strict inchangé) :
--   - p_force = false : identique à 0038 — capacité cible et destinations
--     vérifiées, réorganisation atomique si besoin.
--   - p_force = true : la capacité de la table CIBLE n'est plus bloquante
--     (même philosophie que move_invitation_table, 0008 : « une table normale
--     peut avoir une chaise en plus au besoin, et bloquer un déplacement en
--     plein événement serait plus gênant qu'utile »). Une réorganisation
--     éventuelle reste possible et ses DESTINATIONS restent strictes — on ne
--     déplace jamais quelqu'un vers une autre table déjà pleine. Le forçage
--     est tracé dans audit_logs (details.force = true) pour rester
--     réconciliable avec la capacité officielle de 400 places / absolue 410.
--
-- L'appel sans p_force (lib/guestApprovalDecide.ts, finalisation d'une
-- réservation 0044 à l'approbation) garde exactement le comportement 0038.
-- ============================================================================
...
begin
  ...
  -- v1.41.0 : avec p_force, la table cible peut dépasser sa capacité (chaise
  -- en plus, même règle qu'un déplacement 0008) -- le contrôle ne s'applique
  -- qu'en comportement strict.
  if not coalesce(p_force, false)
     and v_target_occupancy - v_moved_from_target + v_req.nombre_invites > v_table.capacity then
    raise exception 'target_capacity_exceeded';
  end if;

  -- Chaque destination est vérifiée après regroupement de tous ses mouvements.
  -- Reste strict même en forcé : on ne re-remplit jamais une autre table.
  ...
  insert into audit_logs (event_id, action, invitation_id, table_id, agent_id, details)
    values (v_req.event_id, 'guest_approval_assigned', v_inv.id, p_table_id, p_agent_id,
      jsonb_build_object('request_id', p_request_id, 'nom_invite', v_req.nom_invite,
        ...
        -- v1.41.0 : trace du forçage, pour réconcilier avec la capacité
        -- officielle (400 places) après l'événement.
