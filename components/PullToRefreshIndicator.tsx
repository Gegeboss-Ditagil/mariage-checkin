...
export function PullToRefreshIndicator({
  ...
  return (
    <div
      className="flex items-center justify-center gap-2 overflow-hidden text-xs text-text-faint transition-[height] duration-150"
      style={{ height: refreshing ? 36 : Math.min(pullDistance, 60) }}
    >
      <span
        className={
          'h-3 w-3 rounded-full border-2 border-hairline border-t-transparent ' +
