...
### Tests
- Aucun changement de code applicatif dans ce lot (uniquement complétion de déploiement + une contrainte SQL) — pas de nouveau test ; les 14 suites (`node --test`, 125 tests), `npx tsc --noEmit` et `npm run build` restent la référence de v1.27.0, inchangée.
