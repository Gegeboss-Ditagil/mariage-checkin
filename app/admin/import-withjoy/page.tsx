  ...
  return (
    <div className="flex min-h-dvh flex-col">
      ...
      <div className="flex-1 space-y-5 px-4 py-4">
        ...
        {status === 'previewing' && <p className="text-center text-text-faint">Analyse du fichier…</p>}
        {report && !report.ok && (
          ...
          <div className="space-y-4">
            ...
            {!blocked && (
              <div className="space-y-2 rounded-xl2 border-2 border-status-over/30 bg-status-over/5 p-4">
                ...
                <input
                  className="w-full rounded-xl2 border-2 border-status-over/30 bg-surface px-3 py-2"
...
function Stat({ value, label, accent = '' }: { value: number | string; label: string; accent?: string }) {
  return <div className="card py-2"><p className={'text-xl font-bold ' + accent}>{value}</p><p className="text-[11px] text-text-faint">{label}</p></div>;
}
