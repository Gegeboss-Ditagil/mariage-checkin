...
function normalizeAgendaItem<T extends { custom_assignees?: unknown; is_private?: unknown }>(
  item: T
): T & { custom_assignees: string[]; is_private: boolean } {
  return {
    ...item,
    custom_assignees: Array.isArray(item.custom_assignees) ? (item.custom_assignees as string[]) : [],
...
export async function GET() {
  ...
  const canManage = hasCapability(user.role, 'manageAgenda');
  ...
  // Filtrage cote serveur (0058_agenda_items_private.sql) : un role sans
  // manageAgenda (agent scan, agent placeur) ne recoit meme pas la ligne
  // dans le JSON pour un element prive -- jamais un simple masquage cote
  // client, qui laisserait fuiter le contenu dans la reponse reseau.
...
export async function PATCH(req: NextRequest) {
  ...
  for (const key of ['time_label', 'title', 'department', 'details', 'sort_order', 'assignee_ids', 'custom_assignees', 'completed', 'is_private']) {
  ...
  if ('is_private' in updates) updates.is_private = updates.is_private === true;
