...
- Dans `/approbations`, toucher une demande ouvre une fiche remontée près du haut de l'écran : photo en grand, champs distincts Nom/Invités/Côté/Demandé par/Placement et boutons Approuver/Refuser. Deux grandes flèches flottantes en verre iOS permettent de traiter la demande précédente/suivante sans fermer la fenêtre. Le résultat distingue explicitement « Approuvé — sans table » et « Approuvé — Table X ». Après approbation, l'approbateur peut voir les tables recommandées (places réellement libres, places provisoires non arrivées, réserve) ou laisser le placeur décider ; par SMS/WhatsApp, la réponse reste limitée à Oui/Non. Si la table est pleine, le placement impose d'abord de déplacer des groupes non arrivés vers une destination ayant assez de capacité, dans la même transaction.
